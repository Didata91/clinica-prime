...
-- 7) Habilitar RLS em agendamento_status se não estiver habilitado
ALTER TABLE public.agendamento_status ENABLE ROW LEVEL SECURITY;

-- 8) Políticas RLS para agendamento_status (apenas se não existir)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'agendamento_status' 
    AND policyname = 'Anyone can view status options'
  ) THEN
